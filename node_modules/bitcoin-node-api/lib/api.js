  ...
  function express_app(){
    ...
    app.get('*', hasAccess, function(req, res){
      ...
      if(method == "sendmany"){
    	...
    	command = specialApiCase('listunspent',query_parameters); 
      }else if(method == "getinfox"){
        command = specialApiCase('getinfox');
      }
      else{
        command = [{
          method: method,
    ...
    function hasAccess(req, res, next){
      ...
      if('undefined' == typeof accesslist[method]){
        if(accesslist.type == 'only') res.end(method+':This method is restricted.');
        else return next();
      }
      else{
        ...
        else res.end(method+':This method is restricted.');
      } 
    }
    	  ...
    	  for(var parameter in query_parameters){
              if(query_parameters.hasOwnProperty(parameter)){
                if(parameter == 'address'){
                	address[0]=query_parameters[parameter];
                	params.push(address);
                }
              }
    	  }    	   
      }
      if(method_name == 'getinfox'){
    	  return [{
    	        method: 'getinfo',
    	        params: []
    	      }];   	   
